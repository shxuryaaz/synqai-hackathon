...
import { Btn, Card, Icon, I, RuleChip, Section } from './ui'
...
export default function Evaluator({ tick }) {
  ...
  useEffect(() => { get('precedence').then(setPrec); get('graph').then(setG) }, [tick])
  const go = (k, fn) => async () => { setBusy(k); await fn(); setBusy('') }
  return (
    <div className="grid max-w-[1200px] grid-cols-1 gap-5 md:grid-cols-2">
      ...
      <Card className="flex flex-col gap-4 p-6">
        ...
        <div className="flex flex-wrap gap-3"><Btn onClick={go('s', () => post('pii-scan', { plant: false }).then(setScan))} disabled={!!busy}>Run the PII scan</Btn>
          <Btn kind="outline" onClick={go('p', () => post('pii-scan', { plant: true }).then(setPlant))} disabled={!!busy}>Test mode: plant a leak</Btn></div>
      ...
      <Card className="flex flex-col gap-4 p-6">
        ...
        <Btn onClick={go('a', () => get('random-ticket').then(setReplay))} disabled={!!busy} className="self-start">Replay a random ticket</Btn>
