  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <Card className="flex flex-col gap-4">
        <div><h2 className="text-[21px] font-semibold tracking-tight">Runs twice, same bytes</h2><p className="m-0 text-sub">Runs the whole pipeline twice on tickets.json and compares every output file byte for byte.</p></div>
      ...
      <Card className="flex flex-col gap-4">
        <div><h2 className="text-[21px] font-semibold tracking-tight">No personal data leaves</h2><p className="m-0 text-sub">Scans outputs/, audit/ and logs/ for phone, aadhaar and licence numbers. Test mode plants a fake number in a scratch copy to prove the scanner bites.</p></div>
      ...
      <Card className="flex flex-col gap-4">
        <div><h2 className="text-[21px] font-semibold tracking-tight">Every decision has a story</h2><p className="m-0 text-sub">Replays the full audit trail for a random processed ticket.</p></div>
        ...
        {replay && <div className="flex flex-col gap-3"><Section>{replay.ticket_id}</Section>
          {replay.steps.map((s, i) => <div key={i} className="flex flex-col gap-1 border-l border-white/15 pl-3"><div className="flex items-baseline gap-2"><span className="font-semibold">{s.step}</span><span className="text-[13px] text-mute">{when(s.at)} · {s.by}</span></div><div className="text-[15px]">{s.decision}</div>
            {s.rules.length > 0 && <div className="flex flex-wrap gap-1.5">{s.rules.map(r => <RuleChip key={r.id} r={r} />)}</div>}</div>)}</div>}
      </Card>
      <Card className="flex flex-col gap-4">
        <div><h2 className="text-[21px] font-semibold tracking-tight">Sources disagree, precedence decides</h2><p className="m-0 text-sub">Order: {prec?.order.join(' > ')}. Every conflict found in the data, with the loser kept on record.</p></div>
        {prec && <div className="overflow-x-auto"><table className="w-full text-[15px]"><thead><tr className="text-left text-[13px] uppercase tracking-wider text-mute"><th className="py-1 pr-3">Entity</th><th className="pr-3">Fact</th><th className="pr-3">Loser said</th><th className="pr-3">Winner</th></tr></thead>
          <tbody>{prec.conflicts.map((c, i) => <tr key={i} className="border-t border-white/10"><td className="py-2 pr-3 font-mono text-sm">{c.entity}</td><td className="pr-3">{c.key.replace('_', ' ')}</td><td className="pr-3 text-sub">{c.source}: {c.value}</td><td className="pr-3 font-semibold">{c.vs_source}: {c.vs_value}</td></tr>)}</tbody></table>
      ...
      <Card className="flex flex-col gap-4 md:col-span-2">
        <div><h2 className="text-[21px] font-semibold tracking-tight">What the store knows, as a graph</h2><p className="m-0 text-sub">Clients, hubs and the rules that shaped dispatches, from live store queries.</p></div>
