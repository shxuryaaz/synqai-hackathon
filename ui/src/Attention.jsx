...
function Quarantined({ q, onHistory, onChange }) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3.5 rounded-xl border border-red-200 bg-red-soft px-5 py-5">
      ...
      <div className="break-words rounded-lg border border-red-200 bg-white px-3.5 py-2.5 font-mono text-sm text-sub">reason: {q.reason} · {q.detail} · from {safeSource(q.source_file)} · record: {Object.entries(q.record).filter(([k, v]) => v && !k.startsWith('_')).map(([k, v]) => `${k}=${v}`).slice(0, 5).join(' · ')}</div>
      <div className="flex flex-wrap items-end gap-3">
        {q.missing.map(f => <label key={f} className="flex w-full min-w-0 grow flex-col gap-1.5 sm:w-auto sm:min-w-[260px]"><span className="text-sm font-medium text-sub">{LABEL[f] || f}</span>
          <input value={vals[f] || ''} onChange={e => setVals({ ...vals, [f]: e.target.value })} disabled={busy} placeholder="Type it here" className="h-12 min-w-0 rounded-[10px] border-[1.5px] border-[#cdd1d9] bg-white px-3.5 text-base outline-none focus:border-ind" /></label>)}
...
function ReviewAlert({ alert, related, onHistory }) {
  ...
  const id = useId()
  return (
    <div className="flex min-w-0 flex-col gap-3.5 rounded-xl border border-amber-200 bg-amb-soft px-5 py-5">
      ...
      {open && <div id={id} role="region" aria-label={`Review of ${safeSource(alert.source_file)}`} className="rounded-lg border border-amber-200 bg-white px-4 py-4">
        ...
        {related.length > 0 ? <ul className="mb-0 mt-3 flex list-none flex-col gap-2 p-0">
          {related.map(q => <li key={q.ticket_id + q.reason} className="flex flex-col gap-2 rounded-lg bg-bg px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button onClick={() => onHistory(q.ticket_id)} className="min-h-11 shrink-0 self-start px-2 text-sm text-ind sm:self-auto">View history</button>
          </li>)}
